            ...
            case "ArrowLeft":
                {
                    ...
                    if (newState !== oldState) game.newCell();
                    break;
                }
            case "ArrowRight":
                {
...
for (const i of rows)for (const n of i.children)n.classList.add("field-cell--empty");
class Game {
    /**
   *
   ...
   */ status = "idle";
    ...
    moveLeft() {
        for(let i = 0; i < 4; i++){
            ...
            const newCells = [
                ...
                if (a.classList.contains("field-cell--empty") && !b.classList.contains("field-cell--empty")) return 1;
                if (!a.classList.contains("field-cell--empty") && b.classList.contains("field-cell--empty")) return -1;
                return 0;
            });
        ...
        for(let i = 0; i < 4; i++){
            for(let n = 0; n < 4; n++)if (this.state[i][n] === this.state[i][n + 1] && this.state[i][n] !== 0) {
                this.score += this.state[i][n] * 2;
                rows[i].children[n].classList.replace(`field-cell--${this.state[i][n]}`, `field-cell--${this.state[i][n] * 2}`);
                rows[i].children[n].innerHTML = `${this.state[i][n] * 2}`;
                this.state[i][n] *= 2;
                rows[i].children[n + 1].classList.replace(`field-cell--${this.state[i][n + 1]}`, "field-cell--empty");
    ...
    moveRight() {
        ...
        for(let i = 0; i < 4; i++){
            for(let n = 3; n > 0; n--)if (this.state[i][n] === this.state[i][n - 1] && this.state[i][n] !== 0) {
                this.score += this.state[i][n] * 2;
                rows[i].children[n].classList.replace(`field-cell--${this.state[i][n]}`, `field-cell--${this.state[i][n] * 2}`);
                rows[i].children[n].innerHTML = `${this.state[i][n] * 2}`;
                this.state[i][n] *= 2;
    ...
    moveUp() {
        ...
        rows[0].append(newCells[0][0], newCells[1][0], newCells[2][0], newCells[3][0]);
        rows[1].append(newCells[0][1], newCells[1][1], newCells[2][1], newCells[3][1]);
        rows[2].append(newCells[0][2], newCells[1][2], newCells[2][2], newCells[3][2]);
        rows[3].append(newCells[0][3], newCells[1][3], newCells[2][3], newCells[3][3]);
        for(let i = 0; i < 4; i++){
            for(let n = 0; n < 3; n++)if (this.state[n][i] === this.state[n + 1][i] && this.state[n][i] !== 0) {
                this.score += this.state[n][i] * 2;
                rows[n].children[i].classList.replace(`field-cell--${this.state[n][i]}`, `field-cell--${this.state[n][i] * 2}`);
                rows[n].children[i].innerHTML = `${this.state[n][i] * 2}`;
                this.state[n][i] *= 2;
                rows[n + 1].children[i].classList.replace(`field-cell--${this.state[n + 1][i]}`, "field-cell--empty");
    ...
    moveDown() {
        ...
        rows[3].append(newCells[0][3], newCells[1][3], newCells[2][3], newCells[3][3]);
        for(let i = 0; i < 4; i++){
            for(let n = 3; n > 0; n--)if (this.state[n][i] === this.state[n - 1][i] && this.state[n][i] !== 0) {
                this.score += this.state[n][i] * 2;
                rows[n].children[i].classList.replace(`field-cell--${this.state[n][i]}`, `field-cell--${this.state[n][i] * 2}`);
                rows[n].children[i].innerHTML = `${this.state[n][i] * 2}`;
                this.state[n][i] *= 2;
    ...
    newCell() {
        ...
        const usedNum = num();
        const gamePlace = rows[emptyCells[place].row].children[emptyCells[place].cell];
    ...
    lostCheck() {
        if (!this.state.some((a)=>a.some((b)=>b === 0))) {
            if (this.adjacentCells()) return true;
        }
        return false;
    }
    adjacentCells() {
        ...
        for(let i = 0; i < 4; i++)for(let n = 0; n < 3; n++){
            if (this.state[i][n] === this.state[i][n + 1]) sameCellsCount++;
            if (newState[i][n] === newState[i][n + 1]) sameCellsCount++;
        }
        if (sameCellsCount === 0) return true;
    }
    /**
   * @returns {number}
   */ getScore() {
